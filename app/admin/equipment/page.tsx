  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-16 sm:py-20">
      ...
      <section className="mt-10 rounded-xl border border-gsgl-navy/10 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-gsgl-navy">1. Download template</h2>
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="text-xs w-full border-collapse">
            <thead>
              <tr className="bg-gsgl-offwhite">
                {COLUMNS.map((c) => (
                  <th
                    key={c.key}
                    className="border border-gsgl-navy/10 px-3 py-2 text-left font-semibold text-gsgl-navy"
                  >
                    {c.label}
              ...
              <tr>
                {COLUMNS.map((c) => (
                  <td
                    key={c.key}
                    className="border border-gsgl-navy/10 px-3 py-2 text-gsgl-gray italic"
                  >
                    {c.hint}
                  </td>
                ))}
      ...
      <section className="mt-6 rounded-xl border border-gsgl-navy/10 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-gsgl-navy">2. Upload your file</h2>
        <p className="mt-2 text-sm text-gsgl-gray">
          Accepts .xlsx or .xls. The first sheet is used; the first row must be the header.
        </p>

        <div
          className="mt-4 flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gsgl-navy/20 bg-gsgl-offwhite px-6 py-10 text-center transition-colors hover:border-gsgl-gold/60 hover:bg-gsgl-sand/30"
          onClick={() => inputRef.current?.click()}
          onKeyDown={(e) => e.key === "Enter" && inputRef.current?.click()}
          role="button"
          tabIndex={0}
        >
          <p className="text-sm font-medium text-gsgl-navy">Click to choose file</p>
          <p className="mt-1 text-xs text-gsgl-gray">.xlsx or .xls</p>
          <input
            ref={inputRef}
            type="file"
            accept=".xlsx,.xls"
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {parseResult.valid.map((opt) => (
                    <tr key={opt.id} className="even:bg-gsgl-offwhite/50">
                      <td className="border border-gsgl-navy/10 px-3 py-2 font-medium text-gsgl-navy">
                        {opt.name}
                      </td>
                      <td className="border border-gsgl-navy/10 px-3 py-2">
                      ...
                      {["spinProfile", "launchProfile", "feel", "forgivenessLevel", "bias", "weightClass"].map((k) => (
                        <td key={k} className="border border-gsgl-navy/10 px-3 py-2 text-gsgl-gray">
